// ============================================================================
//  AnalyzeDetectorSpectrum_Channel.C
// ----------------------------------------------------------------------------
//  Usage   : 1) root -l AnalyzeDetectorSpectrum_Channel.C 
//            2) root -l 'AnalyzeDetectorSpectrum_Channel.C(true)'
...
//  Main macro
// ============================================================================
void AnalyzeDetectorSpectrum_Channel(bool displayOnly = false)
{   // ------------------------------------------------------------------
    ...
    double ROI_MIN = 820.0;
    double ROI_MAX = 1060.0;
    // --- Peak-fitting settings for the ROI ---
    const int    N_PEAKS     = 5;
    const bool   USE_EXP_BKG = false; //Dùng Bkg exp thì bật true
    std::vector<double> PEAK_GUESS  = {879, 911, 940, 968, 1009};
    std::vector<double> SIGMA_GUESS = {12, 5, 6, 5, 3.5};
    // Peak-free side windows used to estimate the background
    const double BKG_LOW_MIN  = 820.0, BKG_LOW_MAX  = 855.0;   // just before peak 1
    const double BKG_HIGH_MIN = 1035.0, BKG_HIGH_MAX = 1060.0; // just after peak 5
    ...
    // ------------------------------------------------------------------
    TH1F* hSpectrum = new TH1F("hSpectrum", "Detector spectrum;ADC channel;Counts",
    ...
    hZoom->SetTitle(Form("Zoomed spectrum (ROI %.0f - %.0f);ADC channel;Counts",
                          ROI_MIN, ROI_MAX));
    ...
    if (!displayOnly) {
        ...
        ComputePeakAreas(totalFit, fitResult, N_PEAKS, hZoom->GetBinWidth(1));     
...
//  For a Gaussian: Area = Amp * Sigma * sqrt(2*pi)
